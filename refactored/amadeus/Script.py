...
def askContext(time_day, week, season):
...
def prefiltering(features):
	...
	for ind,song in enumerate(s_features):
		...
		if (week ==1 and song[1]>0.6) or (week==2 and song[1]<0.3) or (week ==3 and song[1]<0.5):
			...
			continue
		#mode
		if (season==1 and song[2]>0.6) or (season==2 and song[2]<0.3) or (season==3 and song[2]<0.4) or (season==4 and song[2]>0.75):
			to_delete.append(ind)
			continue
...
def centroid():
    ds_path='./dataset/mss/'
    f=open(ds_path+'centroids.pckl','rb')
    centroids = pickle.load(f)
    f.close()
    return centroids

def centering(songs, features):
...
def creation():
    ...
    features = feature()
    centroids = centroid()
    centering( songs, features )
    return [songs, features, centroids]

def recommendation():
    numpy.set_printoptions(suppress=True)

    [songs, features, centroids, names] = creation()
    ...
    for i in reversed(orchestraSongs):
		listenedFeatures.append(songs[i])
    ...
    model=createModel(listenedFeatures,centroids)

    print "\nAmadeus is computing your recommendations...\n"
    utilities=[]

    for f in features:
        p=dist2prob(f,centroids)
        utilities.append(computeUtility(model,p))

    ind_recomm=selectBestSongs(utilities)
    ...
    [f_features,del_ind]=prefiltering(features)
    ...
    f_ind=selectBestSongs(f_utilities)
    ...
    w_utilities=postfiltering(features,utilities)
    w_ind=selectBestSongs(w_utilities )
    
    wf_utilities=postfiltering(f_features,f_utilities);
    wf_ind=selectBestSongs(wf_utilities);
